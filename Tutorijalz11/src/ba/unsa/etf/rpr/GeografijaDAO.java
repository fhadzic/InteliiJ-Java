...
public class GeografijaDAO {
    ...
    private GeografijaDAO() {
        ...
        try {
            glavniGradUpit = conn.prepareStatement("SELECT g.id, g.naziv, g.broj_stanovnika, g.drzava, g.slika, g.postanski_broj FROM grad g, drzava d WHERE g.drzava=d.id AND d.glavni_grad = ?");
        } catch (SQLException e) {
            ponovoGenerisiBazu();
            try {
                glavniGradUpit = conn.prepareStatement("SELECT g.id, g.naziv, g.broj_stanovnika, g.drzava, g.slika, g.postanski_broj FROM grad g, drzava d WHERE g.drzava=d.id AND d.glavni_grad = ?");
        ...
        try {
            ...
            dajGradUpit = dajGradSaFkDrzavaUpit = conn.prepareStatement("Select g.id, g.naziv, g.broj_stanovnika, g.drzava, g.slika, g.postanski_broj From grad g Where g.id=?;");
            ...
            dajGradSaFkDrzavaUpit = conn.prepareStatement("Select g.id, g.naziv, g.broj_stanovnika, g.drzava, g.slika, g.postanski_broj From grad g Where g.drzava=?;");
            ...
            azurirajGrad = conn.prepareStatement("Update grad Set naziv=?, broj_stanovnika=?, drzava=?, slika=?, postanski_broj=? Where id=?;");
        } catch (SQLException e) {
            e.printStackTrace();
        }
    ...
    private Grad dajGradIzResultSeta(ResultSet rs) throws SQLException {
        Grad grad = new Grad(rs.getInt(1), rs.getString(2), rs.getInt(3), null, rs.getString(5), rs.getString(6));
    ...
    public void dodajGrad(Grad grad) {
        try {
            ...
            dodajGradUpit.setString(6, grad.getPostanskiBroj());
    ...
    public void izmijeniGrad(Grad grad) {
        ...
        try {
            ...
            if (grad.getId() >= 1 && grad.getId() < id) {
                ...
                azurirajGrad.setString(4, grad.getSlika());
                azurirajGrad.setString(5, grad.getPostanskiBroj());
                azurirajGrad.setInt(6, grad.getId());
